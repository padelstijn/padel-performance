...
window.onload = () => {
  ...
  for(let s=1;s<=numSets;s++){
    ...
    for(let i=1;i<=bolletjesPerTeam;i++){
      const bA=document.createElement('div'); bA.className='bolletje'; bA.textContent=i;
      bA.onclick=()=>selectBolletjes(`teamA${s}`,i);
      document.getElementById(`teamA${s}`).appendChild(bA);
      const bB=document.createElement('div'); bB.className='bolletje'; bB.textContent=i;
      bB.onclick=()=>selectBolletjes(`teamB${s}`,i);
      document.getElementById(`teamB${s}`).appendChild(bB);
    }
  }
...
function makeAnalysis(){
  ...
  const ai=aiAnalyse();
  for(let p=1;p<=numPlayers;p++){
    const name=document.getElementById(`player${p}`).value;
    txt+=`Speler ${name}:\n`;
  ...
  for(let s=1;s<=numSets;s++){
    ...
    if(tA+tB>0){
      let winner='Gelijk'; if(tA>tB) winner='Team A'; else if(tB>tA) winner='Team B';
      txt+=`Set ${s}: ${tA} - ${tB} → Winnaar: ${winner}\n`;
    }
  }
